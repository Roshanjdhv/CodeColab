...
export function ExploreTab({ onJoinRoom }: ExploreTabProps) {
  ...
  return (
    <div className="flex flex-col min-h-full">
      {/* Hero Section */}
      <section className="relative bg-white/60 backdrop-blur-xl rounded-3xl py-12 px-6 md:px-12 overflow-hidden mb-12 border border-slate-200/50 shadow-sm">
        <div className="absolute inset-0 z-0 opacity-10 pointer-events-none">
          <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-200 rounded-full filter blur-[100px] -mr-32 -mt-32"></div>
          <div className="absolute bottom-0 left-0 w-48 h-48 bg-purple-200 rounded-full filter blur-[80px] -ml-24 -mb-24"></div>
        </div>
        
        <div className="relative z-10 w-full">
          <div className="mb-10 text-center md:text-left">
            <h2 className="text-3xl md:text-5xl font-extrabold text-slate-900 mb-4 tracking-tight">
              Explore Coding <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600">Rooms</span>
            </h2>
            <p className="text-slate-500 text-base md:text-lg max-w-2xl mx-auto md:mx-0">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-4">
            <div className="relative flex-1 group">
              ...
              <input 
                ...
                className="w-full bg-white border border-slate-200 rounded-2xl py-4 pl-12 pr-4 text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-medium shadow-sm"
                value={searchQuery}
            ...
            <form onSubmit={handleJoinByCode} className="flex gap-2">
              <div className="relative flex-1 md:w-64 group">
                ...
                <input
                  ...
                  className="w-full bg-white border border-slate-200 rounded-2xl py-4 pl-12 pr-4 text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-mono tracking-wider shadow-sm"
                />
              </div>
              <button
                type="submit"
          ...
          <div className="flex justify-center md:justify-start">
             <button 
                ...
                className="w-full md:w-auto bg-indigo-50 hover:bg-indigo-100 text-indigo-700 px-8 py-4 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all border border-indigo-100 active:scale-95 group"
              >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {uniqueRooms.map((room) => {
          ...
          return (
            <div
              key={room._id}
              className="group bg-white rounded-3xl border border-slate-200 overflow-hidden hover:border-indigo-300 transition-all hover:shadow-xl hover:shadow-indigo-500/5 flex flex-col shadow-sm"
            >
              <div 
              ...
              <div className="p-6 flex-1 flex flex-col">
                ...
                <div className="flex items-center gap-2 mb-4 text-slate-500">
                  ...
                  <span className="text-sm truncate">Owner: <span className="text-slate-700 font-medium">{room.creatorName || "Anonymous"}</span></span>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mb-8">
                  <span className="px-3 py-1 rounded-lg bg-slate-100 border border-slate-200 text-slate-600 text-[10px] font-bold uppercase tracking-wider">
                    #{room.language}
                  </span>
                </div>
      ...
      {uniqueRooms.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24 px-6 text-center bg-white rounded-3xl border border-dashed border-slate-200 shadow-sm">
      ...
      {showPrivateRoomModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={() => setShowPrivateRoomModal(false)}></div>
          <div className="bg-white rounded-3xl border border-slate-200 p-8 w-full max-w-md relative z-10 shadow-2xl overflow-hidden group">
            ...
            <div className="text-center relative z-10">
              ...
              <div className="space-y-4">
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-4 px-6 text-center text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-mono tracking-widest text-lg"
                  autoFocus
                />
                <div className="grid grid-cols-2 gap-3 pt-2">
                   <button 
                    onClick={() => setShowPrivateRoomModal(false)}
                    className="py-4 rounded-2xl font-bold text-slate-500 hover:text-slate-900 hover:bg-slate-100 transition-all"
                  >
                    Cancel
                  </button>
                  <button 
