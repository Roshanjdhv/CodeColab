...
export function RequestsTab() {
  ...
  return (
    <div className="flex flex-col min-h-full font-display">
      {/* Header */}
      <header className="flex items-center justify-between border-b border-primary/10 px-6 py-4 bg-white rounded-t-xl">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-xl font-bold leading-tight tracking-tight text-slate-900">Requests</h2>
        </div>
        <div className="flex items-center gap-2">
          <button className="flex h-10 w-10 items-center justify-center rounded-lg bg-slate-50 text-slate-400 hover:bg-slate-100 transition-colors border border-slate-100">
            <Users className="w-5 h-5" />
          </button>
        </div>
      </header>

      <div className="flex flex-col bg-white shadow-sm rounded-b-xl overflow-hidden min-h-[600px]">
        {/* Tabs */}
        <div className="px-6 pt-4 border-b border-slate-100">
          <div className="flex gap-8">
            ...
            <button className="flex flex-col items-center justify-center border-b-2 border-transparent text-slate-500 hover:text-slate-800 pb-3 pt-2">
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-3">
                {friendRequests.map((request) => (
                  <div key={request._id} className="group p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:border-primary/20 hover:bg-white hover:shadow-md transition-all">
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex items-center gap-4">
                        <div className="relative size-12 rounded-xl border-2 border-white overflow-hidden shadow-sm bg-slate-200">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{new Date(request._creationTime).toLocaleDateString()}</p>
                          {request.message && (
                            <p className="text-sm text-slate-600 mt-1 italic line-clamp-1 border-l-2 border-primary/20 pl-2">
                              "{request.message}"
                            </p>
                          )}
                        </div>
                      ...
                      <div className="flex gap-2">
                        <button 
                          ...
                          className="p-2.5 bg-white border border-slate-200 text-slate-400 hover:text-red-500 hover:border-red-100 rounded-xl transition-all active:scale-95 shadow-sm"
                        >
                          <X className="w-5 h-5" />
                        </button>
                        <button 
                          onClick={() => handleFriendRequestAction(request._id, "accept")}
                          className="p-2.5 bg-primary text-white rounded-xl shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all active:scale-95 flex items-center gap-2 px-4"
                        >
            ...
            <div className="space-y-4 pt-4">
              ...
              <div className="grid gap-3">
                {joinRequests.map((request) => (
                  <div key={request._id} className="group p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:border-emerald-200 hover:bg-white hover:shadow-md transition-all">
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in slide-in-from-bottom-4 duration-700">
              <div className="relative mb-8">
                <div className="w-64 h-64 relative flex items-center justify-center rounded-full bg-primary/5">
                  ...
                  <div className="z-10 bg-white p-8 rounded-2xl shadow-xl border border-primary/10">
              ...
              <div className="max-w-[420px] space-y-3">
                <h3 className="text-2xl font-bold text-slate-900 tracking-tight">Everything is up to date</h3>
              ...
              <div className="mt-10 flex flex-col sm:flex-row gap-3">
                <button className="flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-primary text-white font-bold hover:bg-primary/90 transition-all shadow-lg shadow-primary/20 min-w-[160px] active:scale-95">
                  <Share2 className="w-5 h-5" />
                  <span>Invite Friends</span>
                </button>
                <button className="flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-slate-100 text-slate-700 font-bold hover:bg-slate-200 transition-all min-w-[160px] active:scale-95">
